...
def derive_search_terms_from_history(patient_history) -> List[str]:
    """
    Extract symptom-search terms (chief complaints + vitals-derived terms)
    from a ComprehensivePatientHistory object.

    Shared by the public /search/by-symptoms EMR-enrichment path (below)
    and the authenticated, SMART-session-backed ambulatory full-chart
    endpoint in smart_router.py - both need the same chart-to-search-terms
    logic, just with different auth/data-sourcing in front of it.
    """
...
async def rank_diagnoses(
    effective_symptoms: List[str],
    age: Optional[int] = None,
    sex: Optional[str] = None,
    family: Optional[str] = None,
) -> tuple:
    """
    Core ranked-differential logic: tree-based match scoring (rules-first),
    with an AI fallback when tree coverage is sparse or low-confidence.

    Extracted from the /search/by-symptoms route handler so it can be
    reused by any caller that has already assembled its own symptom list -
    in particular the SMART-session-authenticated ambulatory full-chart
    endpoint in smart_router.py, which builds effective_symptoms from a
    live EHR chart pull rather than a request body.

    Returns:
        (SymptomSearchResponse, ai_tree_info | None)
    """
    # Load all families
    ...
    if family:
        ...
        families_to_search = {family: all_families[family]}
    else:
    ...
    for family_name, rules in families_to_search.items():
        # Apply filters
        filtered_rules = apply_filters(rules, age, sex)
    ...
    if should_query_ai and AI_GENERATION_AVAILABLE:
        ...
        try:
            ai_diagnoses = await query_ai_for_diagnoses(effective_symptoms, age, sex)
    ...
    return response_data, ai_tree_info
    ...
    try:
        ...
        if request.emr_patient_id:
            try:
                ...
                _append_unique_terms(effective_symptoms, derive_search_terms_from_history(patient_history))
            except Exception as e:
        ...
        response_data, ai_tree_info = await rank_diagnoses(
            effective_symptoms,
            age=request.age,
            sex=request.sex,
            family=request.family,
